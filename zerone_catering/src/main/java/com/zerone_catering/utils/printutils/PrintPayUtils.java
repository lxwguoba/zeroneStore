package com.zerone_catering.utils.printutils;


import com.zerone_catering.domain.PayOrderDetails;
import com.zerone_catering.print.TableItem;
...
public class PrintPayUtils {
    ...
    public static void print(String name, PayOrderDetails pd, int lian) {
        //-----------------------------最大分割线-----------------------
        ...
        if (pd.getData().getOrdergoods() != null && pd.getData().getOrdergoods().size() > 0) {
            for (int i = 0; i < pd.getData().getOrdergoods().size(); i++) {
                TableItem ti = new TableItem();
                String[] str = {pd.getData().getOrdergoods().get(i).getTitle(), "￥" + pd.getData().getOrdergoods().get(i).getPrice(),
                        "x" + pd.getData().getOrdergoods().get(i).getTotal()};
                ti.setText(str);
                int[] al = {0, 0, 2};
                ti.setAlign(al);
        ...
        String rmsg = "";
        if (pd.getData().getOrderdata().getRemarks() != null && pd.getData().getOrderdata().getRemarks().length() > 0 && !"null".equals(pd.getData().getOrderdata().getRemarks())) {
            rmsg = pd.getData().getOrderdata().getRemarks();
        } else {
            rmsg = "没有备注";
        }
        ...
        String[] orderprintBeanCont = {"订单编号:", pd.getData().getOrderdata().getOrdersn(), ""};
        ...
        String[] orderprintBeanCont01 = {"下单时间:", JavaUtilsNormal.getTime(Long.parseLong(pd.getData().getOrderdata().getCreated_at())), ""};
        ...
        String[] monTiCon = {"原价：", "", "￥" + pd.getData().getOrderdata().getOrder_price()};
        int[] monTialt = {0, 2, 2};
        int[] monTiwid = {2, 0, 3};
        ...
        String dis = "";
        if ("10.00".equals(pd.getData().getOrderdata().getDiscount())) {
            dis = "无";
        } else {
            dis = pd.getData().getOrderdata().getDiscount() + "折";
        }
        ...
        String[] monTiCo = {"实收：", "", "￥" + pd.getData().getOrderdata().getPayment_price()};
        int[] monTial = {0, 2, 2};
        int[] monTiwi = {2, 0, 3};
        ...
        //有备注则打印备注没有怎么不打印
        if (!(pd.getData().getOrderdata().getRemarks() != null
                && pd.getData().getOrderdata().getRemarks().length() > 0
                && !"null".equals(pd.getData().getOrderdata().getRemarks()))) {
