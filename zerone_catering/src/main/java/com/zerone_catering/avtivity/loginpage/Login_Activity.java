...
public class Login_Activity extends BaseActvity {
    ...
    private void gotoLogin() {
        ...
        loginMap.put("terminal_num", "58405257");
        //终端号
        String terminalId = (String) AppSharePreferenceMgr.get(Login_Activity.this, "terminalId", "");
        //pos商户号
        String merchantId = (String) AppSharePreferenceMgr.get(Login_Activity.this, "merchantId", "");
        //POS机终端号
        if (terminalId == null && merchantId == null) {
            Toast.makeText(Login_Activity.this, "该机器没有绑定这个账户，请联系客服。", Toast.LENGTH_SHORT).show();
            return;
        }
        loginMap.put("terminal_num", terminalId);
        //POS商户号
//        loginMap.put("sft_pos_num", merchantId);
        if (!NetworkUtil.isNetworkAvailable(Login_Activity.this)) {
            Toast.makeText(Login_Activity.this, "网络不可用，请检查", Toast.LENGTH_SHORT).show();
            return;
        }
        loading_dailog = LoadingUtils.getDailog(Login_Activity.this, Color.RED, "登录中。。。。");
        loading_dailog.show();
        NetUtils.netWorkByMethodPost(Login_Activity.this, loginMap, IpConfig.URL_LOGIN, handler, 0);
